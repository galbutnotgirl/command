...
REQUIRED_BUNDLE_PATTERNS = [
    ...
    "[agent] ERROR missing restart-app.sh",
    'cp "$UNINSTALLER" "${APP}/Contents/Resources/uninstall-command.sh"',
    "[agent] ERROR missing uninstall-command.sh",
]
REQUIRED_RELEASE_PATTERNS = [
    ...
    '"${DIR}/test/test-build-transaction.sh"',
    '"${DIR}/test/test-release-transaction.sh"',
    '"${DIR}/test/test-install-state.sh"',
    '"${DIR}/test/test-uninstall.sh"',
    '"${DIR}/test/test-updater-swap.sh"',
    '"${DIR}/test/test-restart-app.sh"',
    '"${DIR}/test/test-release-policy.sh"',
    '"${DIR}/test/test-qualification-orchestration.sh"',
    'python3 "${DIR}/test/test-installed-qualification-report.py"',
    '"${DIR}/test/test-static-analysis.sh"',
    'python3 "${DIR}/test/test-docs.py"',
    'python3 "${DIR}/test/test-pages.py"',
    'python3 "${DIR}/test/test_string_review.py"',
    "Swift tests failed",
    "Node tests failed",
    "shell tests failed",
    "docs validation failed",
    "verify-installed-qualification.py",
    ...
    "--publish requires --notarize",
    "--allow-unnotarized is restricted to alpha versions",
    "xcrun notarytool submit",
    "xcrun stapler staple",
    "xcrun stapler validate",
    "Gatekeeper rejected notarized app",
    "Developer ID Application signature",
    "hardened runtime",
    "codesign --verify --deep --strict",
    "./release.sh --publish --notarize",
    "notarizes + staples the app, checks Gatekeeper",
...
REQUIRED_TEST_WORKFLOW_PATTERNS = [
    ...
    "./test/test-shell.sh",
    "Fresh and incremental install state tests",
    "./test/test-install-state.sh",
    "Uninstall ownership and permission-reset tests",
    "./test/test-uninstall.sh",
    "Updater install and rollback tests",
    "./test/test-updater-swap.sh",
    "Restart handoff tests",
    "./test/test-restart-app.sh",
    "Release signing policy tests",
...
BUILT_IN_DOC_LABELS = {
    "add": "Selected text -> Existing conversation",
    "comment": "Selected text -> New conversation",
    "go": "Selected text -> New conversation + auto-submit",
    "shotadd": "Screenshot -> Existing conversation",
    "shotcomment": "Screenshot -> New conversation",
    "shotgo": "Screenshot -> New conversation + auto-submit",
}
...
REQUIRED_TEXT = {
    ...
    "docs/index.html": [
        "<title>Command — Stop copy-pasting into Claude and ChatGPT</title>",
        "href=\"#content\">Skip to content</a>",
        "<main id=\"content\">",
        "install.html",
        "Stop copy-pasting into your AI tools.",
        "Define instructions once. Use every time.",
        "font: 700 14px/1.1 var(--mono)",
        ".history-copy .eyebrow { color: var(--green); }",
        "class=\"history-description\"",
        "Capture or dictate",
        "Send context with it",
        "Works with ChatGPT",
        "Command keeps your clipboard history from the last seven days",
        "Find it again in seconds.",
        "Clipboard History preview",
        "Skip the back-and-forth. Send it with Command.",
        "https://github.com/galbutnotgirl/command/releases/latest",
        "privacy.html",
    ...
    "docs/CHANGELOG.md": [
        ...
        "About includes GitHub, Report a Bug, Request Feature, and Docs routes",
    ...
    "docs/troubleshooting.html": [
        "Screenshot -> New conversation shortcut does nothing",
    ...
    "docs/INSTALL.md": [
        ...
        "legacy SendHelper keystroke fallback",
        "Optional, if you want legacy right-click Services",
        "For isolated unqualified UI development",
        "./script/build_and_run.sh --allow-unqualified --verify",
    ...
    "docs/LIMITATIONS.md": [
        ...
        "New conversation + auto-submit combinations are unbound by default",
    ...
    "agent/SettingsWindow.swift": [
        ...
        "AboutLinkButton(title: \"Report a Bug\"",
        "AboutLinkButton(title: \"Request Feature\"",
        "AboutLinkButton(title: \"Copy Diagnostic Info\"",
        "Uninstall Command…",
        "Uninstall and Keep My Data",
        "Uninstall and Remove Everything",
        "Accessibility, Screen Recording, and Microphone approvals",
        "uninstall-command",
        "try exportGlobalBundle()",
        "Label(\"Export…\"",
        "Label(\"Import…\"",
        ...
        "If macOS asks again after a rebuild, the app was signed with a different identity.",
    ...
    "agent/main.swift": [
        ...
        "handOffRestartAfterExit",
        "reopenAfterExit",
        "bundled restart-app.sh missing",
        "Successful exit",
        "preventing competing instances",
        "restartInProgress",
    ...
    "restart-app.sh": [
        ...
        "launchd unavailable; reopened app",
    ...
    "agent/MenuBar.swift": [
        "statusIconWidth: CGFloat { max(24",
    ...
    "script/build_and_run.sh": [
        'APP_NAME="Command"',
        'ALLOW_UNQUALIFIED=0',
        "Refusing to launch unqualified development build.",
    ...
    ".codex/environments/environment.toml": [
        'name = "claude-command"',
        'command = "./script/build_and_run.sh --allow-unqualified"',
    ],
    ...
    "docs/SUPPORT.md": [
        ...
        "`App path`",
        "Review copied diagnostics for sensitive log or recent-text content",
        "Use [Security Policy](SECURITY.md) instead",
        "Security Policy",
        "private advisory",
    ...
    "uninstall-command.sh": [
        ...
        "unrelated Claude data preserved",
    ],
    "release.sh": [
    ...
    "test/test-release-asset.sh": [
        ...
        "bundled docs/quick-reference.html missing Background Result Contract",
        ...
        'id="existing-alpha"',
        "bundled docs/install.html missing explicit unqualified-development wording",
...
FORBIDDEN_TEXT = {
    "README.md": [
        "./build-agent.sh\n./build-helper.sh\n./install-agent.sh",
        "background handoff",
        "ChatGPT app Codex destination uses",
    ],
    "CONTRIBUTING.md": [
        "macOS 13+",
        "./release.sh\n./release.sh --publish",
    ...
    "agent/main.swift": [
        ...
        "CommandAgent",
        "Restart Agent",
        "NX_SYSDEFINED keyCode=",
    ],
    "agent/Actions.swift": [
        "legacyDictationHomeMigratedToFn",
        "migrateLegacyDictationHomeBindingIfNeeded",
    ],
    "build-agent.sh": [
...
def swift_built_in_compose_rows() -> list[tuple[str, str, str, str]]:
    ...
    for action, input_label, behavior_label in re.findall(
        ...
        delivery = "Existing conversation" if action in {"add", "shotadd"} else "New conversation"
